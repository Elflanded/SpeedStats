  ...
  run(message, args) {
    ...
    if (args.command === "") {
      ...
      embed.addFields(this.client.registry.groups.map(group => ({
        name: group.name,
        value: group.commands.filter(command => !!this.client.owners.find(u => u.id === message.author.id) && command.hasPermission(message.author, false)).map(command => this.client.commandPrefix + command.name).join(", "),
        inline: true
      })));
